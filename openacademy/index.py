...
from openacademy import db, login as login_manager, create_app
...
def init_app(app):
    ...
    def student_register():
        ...
        return render_template('student_register.html', goals=models.StudyGoal, levels=models.StudentLevel,
                               err_msg=err_msg,
    ...
    def payment():
        ...
        vnp_pay.request_data.update({
            ...
            'vnp_Amount': amount, 'vnp_CurrCode': 'VND',
            'vnp_TxnRef': f"{course_id}-{datetime.now().strftime('%H%M%S')}",
        ...
        return redirect(
            vnp_pay.get_payment_url(current_app.config['VNPAY_PAYMENT_URL'], current_app.config['VNPAY_HASH_SECRET']))
    ...
    def payment_return():
        ...
        if vnp_pay.validate_response(current_app.config['VNPAY_HASH_SECRET']) and vnp_pay.response_data[
            'vnp_ResponseCode'] == '00':
            ...
            if not Enrollment.query.filter_by(student_id=current_user.id, course_id=course_id).first():
                db.session.add(Enrollment(student_id=current_user.id, course_id=course_id,
                                          total_payment=float(vnp_pay.response_data['vnp_Amount']) / 100,
                                          payment_status=True))
                db.session.commit()
        ...
        if content or image_url:
            try:
                ...
                return jsonify({
                    ...
                    "user": {"full_name": f"{current_user.last_name} {current_user.first_name}",
...
if __name__ == '__main__':
    from openacademy import create_app
